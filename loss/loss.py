...
class TotalLoss(nn.Module):
    def __init__(self,
        ...
        self.alpha = alpha
        self.beta = beta
        self.focal_loss = FocalLoss(alpha=0.25, mode="binary")
        self.jaccard_loss = JaccardLoss(mode="binary", smooth=1e-6)

    def forward(self, inputs, targets):
        targets = torch.softmax(targets, dim=1)
        loss_focal = self.focal_loss(inputs, targets)
        loss_jaccard = self.jaccard_loss(inputs, targets)

        total_loss = self.alpha*loss_focal + self.beta*loss_jaccard
        return total_loss
